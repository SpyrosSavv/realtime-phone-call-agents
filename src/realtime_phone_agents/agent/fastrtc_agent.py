...
class FastRTCAgent:
    """
    FastRTC agent that encapsulates all dependencies and logic for processing
    audio through speech-to-text, agent reasoning and text-to-speech.
    """
    ...
    def _create_react_agent(
        ...
        """
        Create a LangChain agent with Groq + InMemorySaver + tools.
    ...
    def _build_stream(self) -> Stream:
        """
        Build and configure the FastRTC Stream with the agent hanlder.
